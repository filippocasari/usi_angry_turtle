...
class Move2GoalNode(Node):
    ...
    def move_callback(self):
        ...
        if self.current_pose is None:
            ...
            if self.euclidean_distance(self.pose_target_turtle, self.current_pose) <= self.k2 \
                ...
                print("ANGRY? YES")
                self.tmp_pose = self.current_pose
                ...
                if(self.RETURNING and self.tmp):
                    self.i += (len(self.letters)) % len(self.letters)
                    print('counter : ', str(self.i))
                    self.tmp = False
                    self.set_off_pen()
            ...
            if(self.euclidean_distance(self.pose_target_turtle, self.current_pose) <= self.k1):
                ...
                self.second_turtle_killed = True
                req_kill = Kill.Request()
                req_kill.name = 'turtle2'
                self.create_client(Kill, '/kill').call_async(req_kill)
                self.get_logger().info("SECOND TURTLE KILLED...")
    ...
    def steering_angle(self, goal_pose, current_pose):
        #self.get_logger().info("ANGLE: "+str(atan2(goal_pose.y - current_pose.y, goal_pose.x - current_pose.x)))
        return atan2((goal_pose.y - current_pose.y), (goal_pose.x - current_pose.x))

    def angular_vel(self, goal_pose, current_pose, constant=8.):

        #self.get_logger().info("ANGULAR DIFFERENCE: "+str((self.steering_angle(goal_pose, current_pose) - current_pose.theta)))
